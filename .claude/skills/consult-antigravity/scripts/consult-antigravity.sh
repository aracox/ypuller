...
request="$*"
prompt="You are providing a read-only engineering consultation. Analyze the current repository for the request below. Do not modify files, run commands that mutate state, commit, push, access secrets, or suggest broad unrelated refactors. Return sections for findings, relevant paths, risks, and a focused next-step plan.
